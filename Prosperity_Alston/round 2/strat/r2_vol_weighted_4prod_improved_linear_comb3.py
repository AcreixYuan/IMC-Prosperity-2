    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'PEARLS' or product == 'BANANAS':
                ...
                # coconut_mid = 8/15*pina_mid
                self.acceptable_price[product] = (self.last_mid_price[product]+(self.last_mid_price['PINA_COLADAS']*8/15))/2
            elif product == 'PINA_COLADAS':
                # pina_mid = 15/8*coconut_mid
                self.acceptable_price[product] = (self.last_mid_price[product] +(self.last_mid_price['COCONUTS']*15/8))/2
            else:
                self.acceptable_price[product] = self.last_mid_price[product]
