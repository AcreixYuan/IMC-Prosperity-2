...
# in this case, we use the linear regression result of the data bottle to get the coef

# pina_mid = 2593.3223+1.5512*coconut_mid
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'PEARLS' or product == 'BANANAS':
                ...
                # coconut_mid = (pina_mid-2593.3223)/1.5512
                self.acceptable_price[product] = (self.last_mid_price[product]+((self.last_mid_price['PINA_COLADAS']-2593.3223)/1.5512))/2
            elif product == 'PINA_COLADAS':
                # pina_mid = 2593.3223+1.5512*coconut_mid
                self.acceptable_price[product] = (self.last_mid_price[product] +(self.last_mid_price['COCONUTS']*15512+2593.3223))/2
            else:
                self.acceptable_price[product] = self.last_mid_price[product]
