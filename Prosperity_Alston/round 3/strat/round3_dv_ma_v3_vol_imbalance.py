    ...
    def market_status(self, state: TradingState, product) -> int:
        ...
        if product == 'BERRIES':
            if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                ...
                if -best_ask_volume > 40 and best_bid_volume <= 40:
                    return -1
                elif -best_ask_volume <= 40 and best_bid_volume > 40:
                    return 1
                else:
                    return 0
        elif:
        # TODO: 每个品种的volume imbalance 的threshold 不一样
            pass
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'PEARLS' or product == 'BANANAS':
                ...
                # weighted mid price for pearls and bananas
            ...
            #    self.acceptable_price[product] = self.last_mid_price[product]*(1+np.sin((time%alpha)/alpha*np.pi/10000))
                ...
                if abs(self.last_mid_price['DOLPHIN_SIGHTINGS'] -self.ema_price['DOLPHIN_SIGHTINGS'])>=2.3:
                    # last_mid 大于 ema，说明dolphin sights 迅速上涨
                    # 我们应该把 diving gear的接受价格调高，从而更prone to 买入做多
                    #TODO：这里接受价格的逻辑反了，应该是要买入的时候接受价格调高，要卖出的时候接受价格调低
