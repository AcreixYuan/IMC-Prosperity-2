...
class Trader:
    ...
    def self_compute_orders_basket(self, state: TradingState):
        ...
        spread_5 = self.prices['spread'].rolling(window=5).mean()
        ...
        if not np.isnan(avg_spread.iloc[-1]):
            ...
            # if the past 5 day spread is higher than average spread + 2*std, sell basket
            if spread_5 > avg_spread + std_spread:
                orders_basket.append(Order('GIFT_BASKET', int(round(price_basket))+1, - VOLUME_BASKET))
            ...
            # if the past 5 day spread is lower than average spread - 2*std, buy basket
