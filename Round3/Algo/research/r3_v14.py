...
class Trader:
    ...
    def self_compute_orders_basket(self, state: TradingState):
        ...
        spread_3 = self.prices['spread'].rolling(window=3, min_periods=1).mean()

        if not np.isnan(avg_spread.iloc[-1]):
            avg_spread = avg_spread.iloc[-1]
            std_spread = std_spread.iloc[-1]
            spread_3 = spread_3.iloc[-1]
            logger.print(f"Average spread: {avg_spread}, Spread5: {spread_3}, Std: {std_spread}")
