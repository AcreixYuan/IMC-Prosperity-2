...
class Trader:
    ...
    def self_compute_orders_basket(self, state: TradingState):
        ...
        spread = best_ask_gift_basket - best_bid_roses - 4* best_bid_chocolate - 6* best_ask_strawberries - 380

        # if the spread is positive by 30, full short position of gift basket, full long position of roses, chocolate, strawberries
        if spread > 30:
            orders.append(Order('GIFT_BASKET', best_ask_gift_basket+1, -self.POSITION_LIMIT['GIFT_BASKET'] + position_basket))
            orders.append(Order('ROSES', best_bid_roses-1, self.POSITION_LIMIT['ROSES'] - position_roses))
            orders.append(Order('CHOCOLATE', best_bid_chocolate-1, self.POSITION_LIMIT['CHOCOLATE'] - position_chocolate))
            orders.append(Order('STRAWBERRIES', best_ask_strawberries-1, self.POSITION_LIMIT['STRAWBERRIES'] - position_strawberries))

        
        # if the spread is negative by 30, full long position of gift basket, full short position of roses, chocolate, strawberries
        elif spread < -30:
            orders.append(Order('GIFT_BASKET', best_bid_gift_basket-1, self.POSITION_LIMIT['GIFT_BASKET'] + position_basket))
            orders.append(Order('ROSES', best_ask_roses+1, -self.POSITION_LIMIT['ROSES'] - position_roses))
            orders.append(Order('CHOCOLATE', best_ask_chocolate+1, -self.POSITION_LIMIT['CHOCOLATE'] - position_chocolate))
            orders.append(Order('STRAWBERRIES', best_bid_strawberries+1, -self.POSITION_LIMIT['STRAWBERRIES'] - position_strawberries))
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'GIFT_BASKET':
                try:
                    result[product] = self.self_compute_orders_basket(state)
                except Exception as e:
