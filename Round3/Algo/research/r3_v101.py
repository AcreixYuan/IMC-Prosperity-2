...
class Trader:
    ...
    def self_compute_orders_basket(self, state: TradingState):
        ...
        avg_spread = self.prices['spread'].rolling(window=100, min_periods=1).mean()
        std_spread = self.prices['spread'].rolling(window=100, min_periods=1).std()
