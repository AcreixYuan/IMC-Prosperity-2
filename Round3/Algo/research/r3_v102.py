...
class Trader:
    ...
    def __init__(self):
        self.POSITION_LIMIT = {'AMETHYSTS': 20, 'STARFRUIT': 20, "ORCHIDS": 150, "CHOCOLATE": 250,
    ...
    def self_compute_orders_basket(self, state: TradingState):
        ...
        # calculate the average spread and std of the spread using past 100 periods, if history is not enough, just use all available
        avg_spread = self.prices['spread'].rolling(window=150, min_periods=1).mean()
        std_spread = self.prices['spread'].rolling(window=150, min_periods=1).std()
        ...
        if not np.isnan(avg_spread.iloc[-1]):
            ...
            if spread_5 > avg_spread + std_spread:
