...
class Trader:
    ...
    def __init__(self):
        self.POSITION_LIMIT = {'AMETHYSTS': 20, 'STARFRUIT': 20, "ORCHIDS": 100, "CHOCOLATE": 250,
         "STRAWBERRIES": 350, "ROSE":60, "GIFT_BASKET": 60, "COCONUT": 300, "COCONUT_COUPON": 600}

        self.prices = {
            "spread":pd.Series(),
            "COCONUT": pd.Series(),
            "COCONUT_COUPON": pd.Series(),
        }

        self.ema_price = {'COCONUT': {0: 10000}, 'COCONUT_COUPON': {0: 637.5}}
    ...
    def self_compute_orders_orch(self, state: TradingState):        
        ...
        orders.append(Order('ORCHIDS', int(round((ask_tilt)))+1, -int(round(self.POSITION_LIMIT['ORCHIDS']))))
    ...
    def BS_quick(self, S=10000, K=10000, vol=0.0101193, T=250, r=0, q=0, delta_flag=False):
        d1 = (np.log(S / K) + (r - q + 0.5 * vol ** 2) * T) / (vol * np.sqrt(T))
        d2 = d1 - vol * np.sqrt(T)

        normcdf = lambda x: (1 + math.erf(x / np.sqrt(2))) / 2
        N1=normcdf(d1)
        N2=normcdf(d2)

        px=S*np.exp(-q*T)*N1-K*np.exp(-r*T)*N2
    ...
    def fit_BS_vol(self, vol_fit=0.01, S=9990, px=620,T=250, step=0.00001):
        for i in range(30):
            px_new=self.BS_quick(S=S, vol=vol_fit, T=T)
            if abs(px_new-px)<0.01:
                break
            vol_fit = vol_fit + (px-px_new)*step
        return vol_fit
    
    def self_compute_orders_coconut(self, state: TradingState):
        p_coco = self.get_position(state, 'COCONUT')
        bid_volume_coco = - max(p_coco, 0) + self.POSITION_LIMIT['COCONUT']
        ask_volume_coco = max(-p_coco, 0) - self.POSITION_LIMIT['COCONUT']
        p_coupon = self.get_position(state, 'COCONUT_COUPON')
        bid_volume_coupon = - max(p_coupon, 0) + self.POSITION_LIMIT['COCONUT_COUPON']
        ask_volume_coupon = max(-p_coupon, 0) - self.POSITION_LIMIT['COCONUT_COUPON']
        ...
        # get mid price of coconut and coupon
        price_coconut = self.get_mid_price(state, 'COCONUT')
        price_coupon = self.get_mid_price(state, 'COCONUT_COUPON')

        # save the price of coconut and coupon
        self.save_prices("COCONUT", price_coconut)
        self.save_prices("COCONUT_COUPON", price_coupon)

        # calculate ema price of coconut and coupon
        ema_coconut = (2 * price_coconut + (N1 - 1) * self.ema_price['COCONUT'][list(self.ema_price['COCONUT'].keys())[-1]]) / (N1 + 1)
        ema_coupon = (2 * price_coupon + (N2 - 1) * self.ema_price['COCONUT_COUPON'][list(self.ema_price['COCONUT_COUPON'].keys())[-1]]) / (N2 + 1)

        self.ema_price['COCONUT'][state.timestamp] = ema_coconut
        self.ema_price['COCONUT_COUPON'][state.timestamp] = ema_coupon

        # fit the current volatility of COCONUT using ema price
        volatility = self.fit_BS_vol(vol_fit=0.01, S=ema_coconut, px=ema_coupon, T=250)

        # calculate the theoretical fair price and delta of COCONUT_COUPON
        price_coupon_fair, delta_coconut = self.BS_quick(S=price_coconut, K=10000, vol=volatility, T=250, delta_flag=True)

        best_ask_coconut, best_bid_coconut, best_ask_volume_coconut, best_bid_volume_coconut = self.get_info_product(state, 'COCONUT')
        best_ask_coupon, best_bid_coupon, best_ask_volume_coupon, best_bid_volume_coupon = self.get_info_product(state, 'COCONUT_COUPON')
        logger.print(f"price_coconut: {price_coconut}, price_coupon: {price_coupon}, price_coupon_fair: {price_coupon_fair}, delta_coconut: {delta_coconut}")

        # if the current price of COCONUT_COUPON is lower than the fair price, buy COCONUT_COUPON and sell COCONUT to hedge
        if best_ask_coupon < price_coupon_fair - DEVIATION:
            #find volume coupon adjusted
            bid_vol_cross_coco =  -best_ask_volume_coconut
            bid_vol_cross_coupon =  - best_bid_volume_coupon
            if delta_coconut* bid_vol_cross_coupon < bid_vol_cross_coco:
                orders_coupon.append(Order('COCONUT_COUPON', best_ask_coupon, bid_vol_cross_coupon))
                orders_coconut.append(Order('COCONUT', best_bid_coconut, -int(round(bid_vol_cross_coupon * delta_coconut))))
            else:
                orders_coupon.append(Order('COCONUT_COUPON', best_ask_coupon, bid_vol_cross_coco))
                orders_coconut.append(Order('COCONUT', best_bid_coconut, -int(round(bid_vol_cross_coco * delta_coconut))))
        
        # if the current price of COCONUT_COUPON is higher than the fair price, sell COCONUT_COUPON and buy COCONUT to hedge
        elif best_bid_coupon > price_coupon_fair + DEVIATION:
            ask_vol_cross_coco = best_bid_volume_coconut
            ask_vol_cross_coupon = best_ask_volume_coupon
            if delta_coconut * ask_vol_cross_coupon < ask_vol_cross_coco:
                orders_coupon.append(Order('COCONUT_COUPON', best_bid_coupon, -ask_vol_cross_coco))
                orders_coconut.append(Order('COCONUT', best_ask_coconut, int(round(ask_vol_cross_coco * delta_coconut))))
            else:
                orders_coupon.append(Order('COCONUT_COUPON', best_bid_coupon, -ask_vol_cross_coupon))
                orders_coconut.append(Order('COCONUT', best_ask_coconut, int(round(ask_vol_cross_coupon * delta_coconut))))                
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'COCONUT':
                try:
                    result['COCONUT'], result['COCONUT_COUPON'] = self.self_compute_orders_coconut(state)
                except Exception as e:
                    logger.print(f'COCONUT error: {e}')
