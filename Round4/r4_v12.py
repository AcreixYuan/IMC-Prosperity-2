...
class Trader:
    ...
    def self_compute_orders_coconut(self, state: TradingState):
        ...
        ma_coconut = self.prices['COCONUT'].rolling(window=3, min_periods=1).mean()
        ma_coupon = self.prices['COCONUT_COUPON'].rolling(window=3, min_periods=1).mean()
        ...
        volatility = self.fit_BS_vol(vol_fit=0.01, S=ma_coconut, px=ma_coupon, T=250)
